...
void SRenderDebugGUI::Update(sf::RenderWindow &window,
                             sf::Time DeltaTime,
                             std::unique_ptr<Registry> &Registry,
                             const CCamera &cameraActor)
{
    ImGui::SFML::Update(window, DeltaTime);
    static int NewXPos = 100;
    static int NewYPos = 100;
    static float NewScale = 1.0f;
    static float NewRotation = 0.0f;
    static float NewXSpeed = 100.f;
    static float NewYSpeed = 0.f;
    ...
    if (ImGui::Begin("Create Enemies"))
    {
        ...
        if (ImGui::Button("Create"))
        {
            ...
            Tank.Group("Enemies");
            Tank.AddComponent<CTransform>(sf::Vector2f(NewXPos, NewYPos),
